...
- `System Canvas` is now exposed as a writable agent-native surface for config patching, view switching, scale-graph generation, and theme-var application, with structured manifest/state resources, app-owned screenshot capture, and local CLI + MCP write paths.
...
- Event log for workspace mutations.
- Multi-surface tool routing in MCP.
- Deeper `System Canvas` graph-level mutations beyond config/view/generate/apply.
