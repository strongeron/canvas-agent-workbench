...
export const AGENT_NATIVE_WORKSPACE_DEFINITIONS: AgentWorkspaceDefinition[] = [
  {
    ...
    tools: [
      ...
      },
      {
        ...
        description:
          "Apply one structural HTML/TSX mutation by canvasId through the AST write path. Supports insertChild, removeNode, reorderSibling, wrapSelection, unwrap, swapTag, setTextChild, and replaceChildren.",
        status: "ready",
      },
      {
      ...
      },
      {
        ...
        description: "Read the append-only event log for the current Canvas workspace. Cursor-paged: pass sinceCursor to fetch only newer events, poll again with the returned nextCursor. Event kinds: user-action, source-edit, file-lifecycle, operation-queued, operation-applied, state-synced.",
        status: "ready",
      },
      {
      ...
      {
        id: "get_workspace_manifest",
        title: "Get workspace manifest",
        description: "Read the global agent-native manifest describing every runtime, workspace, resource, tool, and prompt.",
        status: "ready",
      },
      {
        id: "get_surface_manifest",
        title: "Get surface manifest",
        description: "Read the per-surface manifest for canvas, color-audit, system-canvas, or node-catalog.",
        status: "ready",
      },
      {
        id: "list_design_tokens",
        title: "List design tokens",
        description: "Read project-level tokens.css custom properties for the active project, returning current values and mtimeMs for guarded writes.",
        status: "ready",
      },
      {
        id: "update_design_token",
        title: "Update design token",
        description: "Set one CSS custom property in projects/<projectId>/tokens.css through the token writer with an mtime guard.",
        status: "ready",
        destructive: true,
      },
      {
        id: "read_html_node",
        title: "Read HTML node",
        description: "Read tag, attributes, classes, and text for a selected HTML element by canvasId (file-backed or inline).",
        status: "ready",
      },
      {
        id: "update_html_node",
        title: "Update HTML node",
        description: "Apply literal HTML mutations (setClassName, setAttribute, setTextContent) by canvasId through the AST write path with mtime guard.",
        status: "ready",
        destructive: true,
      },
      {
        id: "create_component_from_html",
        title: "Create component from HTML",
        description: "Write a new HTML component file under projects/<projectId>/components/ from pasted HTML/CSS and place a preview node unless createItem is false.",
        status: "ready",
        destructive: true,
      },
      {
        id: "create_component_from_tsx",
        title: "Create component from TSX",
        description: "Write a new React TSX component file under projects/<projectId>/components/ from pasted source and place a preview node unless createItem is false.",
        status: "ready",
        destructive: true,
      },
      {
        id: "promote_to_component",
        title: "Promote subtree to component",
        description: "Extract an HTML subtree (by canvasId) from a canvas item and save it as a new project component. The source item is unchanged.",
        status: "ready",
        destructive: true,
      },
      {
        id: "get_canvas_context",
        title: "Get canvas context",
        description: "Read a compact context summary of the current board: artboards, children, loose items, selection, theme, and item counts by type.",
        status: "ready",
      },
      {
        id: "get_canvas_state",
        title: "Get canvas state",
        description: "Read the raw canvas state snapshot (items, groups, selectedIds, nextZIndex) for the active workspace.",
        status: "ready",
      },
      {
        id: "get_canvas_selection",
        title: "Get canvas selection",
        description: "Read the current selected item ids plus normalized summaries for each selected item.",
        status: "ready",
      },
      {
        id: "list_primitives",
        title: "List primitives",
        description: "List registered primitives for the current project, optionally filtered by family, level, or exportable.",
        status: "ready",
      },
      {
        id: "get_primitive",
        title: "Get primitive",
        description: "Read one primitive by primitiveId or entryId, including variants and prop schema.",
        status: "ready",
      },
      {
        id: "create_artboard",
        title: "Create artboard",
        description: "Create a new artboard item and queue it onto the canvas through the standard create_item operation.",
        status: "ready",
      },
      {
        id: "create_primitive_item",
        title: "Create primitive item",
        description: "Create a component item bound to a registered primitive variant and queue it onto the canvas.",
        status: "ready",
      },
      {
        id: "export_board",
        title: "Export board",
        description: "Export a primitive-only artboard as React code (react-tailwind or react-css-vars). Preserves canvas positioning rather than reconstructing semantic layout.",
        status: "ready",
      },
      {
        id: "duplicate_items",
        title: "Duplicate items",
        description: "Duplicate one or more canvas items with a position offset and fresh ids. Mirrors the UI cmd-D duplicate path and returns { newIds }.",
        status: "ready",
      },
      {
      ...
      {
        id: "redo_source_mutation",
        title: "Redo source mutation",
        description: "Redo the most recently undone source mutation for the active file (cmd-shift-Z parity). Re-applies the post-snapshot via the existing AST writer.",
        status: "ready",
        destructive: true,
      },
      {
    ...
    prompts: [
      ...
      {
        id: "replace-html-bundle",
        title: "Replace HTML bundle",
        description: "Guide an agent through atomically replacing an existing live HTML iframe node by passing targetItemId to import_html_bundle, then verifying with a screenshot.",
        status: "ready",
      },
    ],
  },
  {
    ...
    tools: [
      ...
      },
      {
        id: "get_workspace_events",
        title: "Get workspace events",
        description: "Read the append-only event log for the current Color Audit workspace. Cursor-paged: pass sinceCursor to fetch only newer events, poll again with the returned nextCursor. Event kinds: user-action, source-edit, file-lifecycle, operation-queued, operation-applied, state-synced.",
        status: "ready",
      },
      {
        id: "get_workspace_debug",
  ...
  },
  {
    ...
    tools: [
      ...
      },
      {
        id: "get_workspace_events",
        title: "Get workspace events",
        description: "Read the append-only event log for the current System Canvas workspace. Cursor-paged: pass sinceCursor to fetch only newer events, poll again with the returned nextCursor. Event kinds: user-action, source-edit, file-lifecycle, operation-queued, operation-applied, state-synced.",
        status: "ready",
      },
      {
        id: "get_workspace_debug",
  ...
  },
  {
    ...
    tools: [
      ...
      },
      {
        id: "get_workspace_events",
        title: "Get workspace events",
        description: "Read the append-only event log for the current Node Catalog surface. Cursor-paged: pass sinceCursor to fetch only newer events, poll again with the returned nextCursor. Event kinds: user-action, source-edit, file-lifecycle, operation-queued, operation-applied, state-synced.",
        status: "ready",
      },
      {
        id: "get_workspace_debug",
