...
import { createAgentSearch } from './server/agentSearch.mjs'
import { createAgentSearchRoutes } from './server/agentSearchRoutes'
import { createEmbedMedia } from './server/embedMedia.mjs'
import { createProjectScan } from './server/localScan.mjs'
...
// FOX2-75 slice 9: the projects + local-scan subsystem lives in
// server/localScan.mjs; the vite alias plugin and watchers below consume
// these exports.
const {
  uniqueName,
  ...
  LOCAL_SCAN_PROXY_SOURCE_PATH: path.resolve(__dirname, 'components/local-scan/LocalScannedComponentProxy.tsx'),
  slugify,
  toPascalCase,
  ensureProjectCanvasDir,
})

// FOX2-75 slice 6: the /api/canvas/* tool endpoint group lives in
// server/canvasToolRoutes.ts. The vite-coupled React preview compile and
// the localhost guard stay here and are injected.
const handleCanvasToolRoutes = createCanvasToolRoutes({
...
const { searchWeb, getRoutePlan, searchAssets, inferMediaKindFromMimeType, filenameFromRemoteUrl } =
  createAgentSearch({
    TAVILY_API_KEY,
...
// FOX2-75 slice 8: embed/media capture machinery lives in
// server/embedMedia.mjs; consts and screenshot utils are passed as config.
const {
  normalizeOrigin,
...
const handleAgentSearchRoutes = createAgentSearchRoutes({
  agentSearch: { searchWeb, getRoutePlan, searchAssets },
  importAssetFromRemoteUrl,
})
  ...
  return {
    ...
    async configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        // FOX2-75 slice 6: the /api/canvas/* tool endpoint group is
        // handled by server/canvasToolRoutes.ts.
        if (await handleCanvasToolRoutes(req, res, pathname)) return
        ...
        if (await handleAgentSearchRoutes(req, res, pathname)) return
