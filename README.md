...
A portable component gallery and canvas-based design playground, extracted from the Thicket marketplace-courses project.
...
### Reference (Thicket-specific)
- 📦 **ComponentRenderer** - Full Thicket component map (for reference)
- 📦 **LayoutRenderer** - Thicket layout preview (for reference)
- 📦 **SonnerPreview** - Thicket toast preview (for reference)
- 📦 **EarlyAccessFormPreview** - Thicket form preview (for reference)

## Quick Start

```tsx
// 1. Import from gallery-poc
import {
  GalleryProvider,
  createStaticAdapter,
  PortableGalleryPage,
...
## Integration with Thicket

This POC is extracted from the Thicket gallery system. The existing `.gallery.ts` config files are compatible with the portable version. To migrate:

1. Import types from `gallery-poc/core` instead of `@/platform/gallery/registry/types`
2. Wrap your app with `<GalleryProvider adapter={...}>`
3. Use `PortableComponentRenderer` instead of `ComponentRenderer`
