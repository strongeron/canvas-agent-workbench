...
export function CourseCardReadOnly({ course, teacherId }: CourseCardReadOnlyProps) {
  const courseUrl = teacherId
    ? `${course_path(course.id)}?teacher_id=${teacherId}`
    ...
    <Link
      ...
      onStart={() =>
        track("course_card_details_clicked", {
          props: { course: course.title, role: "teacher" },
        })
      }
