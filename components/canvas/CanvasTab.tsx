...
import {
  serializeHtmlBundleFileEntries,
  serializeHtmlBundleFiles,
  useCanvasAddHandlers,
} from "../../hooks/useCanvasAddHandlers"
import { useCanvasFilePersistence } from "../../hooks/useCanvasFilePersistence"
...
export function CanvasTab({
  ...
  // Artboard add-menu (FOX2-59 method 4); the media-choice file input and
  // its target-artboard ref live in useCanvasAddHandlers.
  ...
  // queue, dirty tracking + autosave (FOX2-40), and the file CRUD handlers
  // with their modal state. Called here (not earlier) because it needs
  // emitFileLifecycle from the agent bridge; called before
  // useCanvasAddHandlers, which consumes runCanvasPersistenceTask.
  const {
  ...
  // Placement helpers + add handlers for every asset kind (FOX2-62 Scale-1
  // PR 2): where a new node lands and how each "Add …" entry point creates it.
  const {
  ...
  const handleAddNativeComponent = useCallback(
    async (
      ...
      if (targetArtboardId) {
        const siblings = items.filter(
          (item) => item.parentId === targetArtboardId && item.type !== "artboard"
        )
        const maxOrder = siblings.reduce((max, item) => Math.max(max, item.order ?? 0), -1)
        ...
        <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          ...
          >
              <CanvasSidebar
                ...
                onRenameCanvasFile={handleRenameCanvasFile}
                onDuplicateCanvasFile={handleDuplicateCanvasFile}
                onDeleteCanvasFile={handleDeleteCanvasFile}
